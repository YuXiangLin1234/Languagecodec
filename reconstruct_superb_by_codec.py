...
def main(args):
    ...
    for i, source in enumerate(audio_files):
        ...
        _, discrete_code= languagecodec.encode_infer(source_audio, bandwidth_id=bandwidth_id)
        features = languagecodec.codes_to_features(discrete_code[-1, :, :])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ref-path", type=str, default="/work/yuxiang1234/backup/codec_eval_data_ref_path")
    parser.add_argument("--syn-path", type=str, default="lc_syn_path/default-1b")
    parser.add_argument("-at", "--ablation-type", type=str, choices=["zero", "avg", "random"], default="zero")
    parser.add_argument('-p', "--without-prosody", action="store_true" ,default=False)
    parser.add_argument('-c', "--without-content", action="store_true" ,default=False)
    parser.add_argument('-a', "--without-acoustic", action="store_true" ,default=False)
    parser.add_argument('-t', "--without-timbre", action="store_true" ,default=False)
